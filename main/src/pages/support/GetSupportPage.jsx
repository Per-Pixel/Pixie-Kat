...
const GetSupportPage = () => {
  ...
  const channels = [
    ...
    {
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" className="size-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {quickHelp.map((item, i) => (
              <motion.div
                ...
                className={`cursor-pointer rounded-2xl border p-5 transition-shadow duration-200 ${item.color}`}
              >
                <div className={`size-11 rounded-xl ${item.iconBg} mb-4 flex items-center justify-center`}>
                  {item.icon}
                </div>
                <h3 className="mb-1.5 text-[0.9375rem] font-semibold">{item.title}</h3>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            {channels.map((ch, i) => (
              <motion.div
                ...
                variants={fadeUp}
                whileHover={{ y: -4 }}
                className={`cursor-pointer rounded-2xl border p-6 transition-all duration-200 ${ch.bgColor}`}
              >
                <div className={`size-12 rounded-xl ${ch.iconBg} ${ch.color} mb-4 flex items-center justify-center`}>
                  {ch.icon}
                </div>
                <h3 className={`text-[0.9375rem] font-semibold ${ch.color} mb-0.5`}>{ch.title}</h3>
                <p className="text-sm text-gray-700">{ch.subtitle}</p>
                <p className="mt-1 text-xs text-gray-400">{ch.detail}</p>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <div className="mx-auto max-w-3xl px-6 md:px-10">
          ...
          >
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
              <div>
                <label htmlFor="gs-name" className="mb-1.5 block text-sm font-medium text-gray-700">
                  Full Name
                </label>
                <input
                  id="gs-name"
              ...
              <div>
                <label htmlFor="gs-email" className="mb-1.5 block text-sm font-medium text-gray-700">
                  Email
                </label>
                <input
                  id="gs-email"
            ...
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
              <div>
                <label htmlFor="gs-order" className="mb-1.5 block text-sm font-medium text-gray-700">
              ...
              <div>
                <label htmlFor="gs-category" className="mb-1.5 block text-sm font-medium text-gray-700">
                  Issue Category
                </label>
                <select
                  id="gs-category"
                  ...
                  onChange={handleChange}
                  required
                  className="w-full appearance-none rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm text-gray-900 transition focus:border-transparent focus:outline-none focus:ring-2 focus:ring-violet-300"
                >
            ...
            <div>
              <label htmlFor="gs-desc" className="mb-1.5 block text-sm font-medium text-gray-700">
                Describe Your Issue
              </label>
              <textarea
                id="gs-desc"
