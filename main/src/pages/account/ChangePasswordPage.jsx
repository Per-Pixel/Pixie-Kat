    ...
    <div className="space-y-2">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="h-14 w-full rounded-2xl border border-white/50 bg-white/60 px-12 text-base font-medium text-slate-900 outline-none transition focus:border-[#8b6dff] focus:bg-white focus:ring-4 focus:ring-[#8b6dff]/10"
        />
        {showToggle && (
          <button
            type="button"
...
const PasswordRule = ({ met, label }) => (
  <div className={`flex items-center gap-2 text-sm transition-colors ${met ? "text-emerald-600" : "text-slate-400"}`}>
    <div className={`size-1.5 shrink-0 rounded-full ${met ? "bg-emerald-500" : "bg-slate-300"}`} />
    {label}
  </div>
);
            ...
            {success ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <button
                  ...
                  className="flex h-14 w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-[#6c49ff] to-[#8b6dff] text-lg font-bold text-white shadow-[0_14px_28px_rgba(108,73,255,0.3)] transition hover:scale-[1.02] disabled:cursor-not-allowed disabled:opacity-60 disabled:hover:scale-100"
                >
                  {saving
