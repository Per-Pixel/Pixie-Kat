ALTER TABLE public.orders
  ADD COLUMN unit_selling_price NUMERIC(14, 2),
  ADD COLUMN unit_cost_price NUMERIC(14, 2);

ALTER TABLE public.orders
  ADD CONSTRAINT orders_unit_selling_price_nonnegative CHECK (unit_selling_price IS NULL OR unit_selling_price >= 0),
  ADD CONSTRAINT orders_unit_cost_price_nonnegative CHECK (unit_cost_price IS NULL OR unit_cost_price >= 0);
...
BEGIN
  ...
  SELECT CASE
    WHEN p.currency = NEW.currency THEN p.cost_price
    ELSE NULLIF(p.metadata #>> ARRAY['currencies', NEW.currency, 'cost_price'], '')::NUMERIC
  END
  INTO v_cost
  FROM public.products p
  WHERE p.id = NEW.product_id;

  NEW.unit_selling_price := NEW.total_amount / NEW.quantity;
  NEW.unit_cost_price := v_cost;
  RETURN NEW;
END;
$$;

CREATE TRIGGER snapshot_order_prices_before_insert
BEFORE INSERT ON public.orders
FOR EACH ROW EXECUTE FUNCTION public.snapshot_order_prices();

CREATE INDEX idx_orders_analytics_period
  ON public.orders(created_at DESC, status, user_id);
...
REVOKE ALL ON FUNCTION public.get_admin_analytics(TIMESTAMPTZ, TIMESTAMPTZ, TEXT) FROM PUBLIC;
