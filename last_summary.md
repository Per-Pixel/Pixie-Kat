...
- [x] Prior-session leftover files now landed: `dba3b7b` feat(server) deploy tooling (`package.json` scripts + `scripts/check-deploy-drift.mjs`), `d1a6a63` fix(batch-order) gateway non-JSON resilience. Reviewed before landing; verified `npm run deploy:check` exits 0 and `vite build` succeeds. Pushed; tree clean, in sync with origin.
- [ ] `check-deploy-drift.mjs` timezone caveat: the EB version label (`app-YYMMDD_HHMMSS…`) appears stamped in the instance's local time (IST, ap-south-1) not UTC as the script assumes — today it compared correctly, but the ~5.5h skew could mask real same-day drift. Worth fixing the parse if drift-check ever false-negatives.
